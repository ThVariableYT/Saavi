...
export default function Oreo() {
  return (
    <div className="relative h-full w-full overflow-hidden flex flex-col items-center justify-center px-5 sm:px-8 vignette"
      style={{
        background:
          "radial-gradient(circle at 30% 24%, #d6e4f0 0%, transparent 50%), radial-gradient(circle at 74% 76%, #c8d8e8 0%, transparent 45%), linear-gradient(165deg, #e8f0f7 0%, #d2e0ec 100%)",
      }}
    >
      <GlowOrb className="absolute w-[30rem] h-[30rem] top-0 left-0" color="rgba(70,110,160,0.16)" />
      <AmbientMotes count={16} color="rgba(70,110,160,0.5)" />

      {PAWS.map((p, i) => (
        <Paw key={i} className={`absolute w-6 h-7 text-slate-500/25`} style={{ top: p.top, left: p.left as string, right: p.right as string, rotate: `${p.rot}deg` }} delay={p.delay} />
      ))}

      <Sunflower className="absolute w-11 h-11 sm:w-14 sm:h-14 opacity-85" style={{ top: "7%", left: "6%" }} delay={0.2} />
      <Sunflower className="absolute w-9 h-9 sm:w-12 sm:h-12 opacity-75" style={{ bottom: "12%", right: "7%" }} delay={0.5} />
      <Sunflower className="absolute w-8 h-8 sm:w-10 sm:h-10 opacity-70" style={{ top: "54%", left: "3%" }} delay={0.8} />
      <LegoBrick className="absolute w-9 h-7 opacity-75" style={{ top: "20%", right: "12%" }} delay={0.3} color="#2A9D8F" />
      <LegoBrick className="absolute w-8 h-6 opacity-70" style={{ bottom: "20%", left: "10%" }} delay={0.6} color="#F4A261" />
      <CuteAnimal type="bunny" className="absolute w-11 h-12 sm:w-14 sm:h-16 opacity-80" style={{ bottom: "8%", left: "5%" }} delay={0.4} />
      <CuteAnimal type="fox" className="absolute w-9 h-9 sm:w-12 sm:h-12 opacity-75" style={{ top: "36%", right: "5%" }} delay={0.9} />
      <Sparkle className="absolute w-3 h-3 text-blue-500" style={{ top: "30%", left: "26%" }} delay={0.7} />
      <Sparkle className="absolute w-4 h-4 text-blue-400" style={{ bottom: "34%", right: "24%" }} delay={2} />

      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="relative z-10 text-center mb-4">
        <p className="font-elegant tracking-[0.4em] uppercase text-slate-600/80 text-[0.6rem] sm:text-xs mb-1.5">In Loving Memory</p>
        <h2 className="font-serif-display text-slate-800" style={{ fontSize: "clamp(1.7rem, 4.8vw, 2.8rem)" }}>Oreo</h2>
      </motion.div>
      ...
      <div className="relative z-10 w-full max-w-4xl flex flex-col sm:flex-row items-center gap-6 sm:gap-10">
        <motion.div initial={{ opacity: 0, scale: 0.9, rotate: -3 }} animate={{ opacity: 1, scale: 1, rotate: -2 }} transition={{ duration: 0.9, delay: 0.15, type: "spring", bounce: 0.3 }} className="relative shrink-0">
          <div className="relative rounded-lg p-2.5 pb-4" style={{ background: "linear-gradient(180deg, rgba(255,255,255,0.85) 0%, rgba(255,255,255,0.55) 100%)", boxShadow: "0 25px 60px -15px rgba(50,80,120,0.3), inset 0 0 0 1px rgba(255,255,255,0.8)", backdropFilter: "blur(8px)" }}>
            <div className="overflow-hidden rounded" style={{ width: "min(58vw, 14rem)", aspectRatio: "3 / 4" }}>
              <img src="/photos/oreo2.jpeg" alt="Oreo the dog" className="w-full h-full object-cover" />
            </div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }} className="flex-1">
          <div className="relative p-5 sm:p-6 rounded-xl" style={{ background: "linear-gradient(160deg, rgba(255,255,255,0.7), rgba(255,255,255,0.4))", boxShadow: "inset 0 0 0 1px rgba(70,110,160,0.25), 0 8px 40px -12px rgba(50,80,120,0.2)", backdropFilter: "blur(22px) saturate(160%)", WebkitBackdropFilter: "blur(22px) saturate(160%)" }}>
            <span className="font-script text-slate-700 text-2xl sm:text-3xl">in his memory</span>
            <div className="font-elegant text-slate-800/90 space-y-3 mt-2 text-center sm:text-left thin-scrollbar" style={{ fontSize: "clamp(1.02rem, 1.8vw, 1.16rem)", lineHeight: 1.8, maxHeight: "44vh", overflowY: "auto" }}>
              ...
              <p className="font-script text-slate-700 text-xl sm:text-2xl pt-1">Forever a good boy. Forever missed. Forever ours.</p>
            </div>
          </div>
        </motion.div>
      </div>
